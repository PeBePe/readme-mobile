...
class EditWishlistPage extends StatefulWidget {
  final id;
  final note;
  const EditWishlistPage(this.id, this.note, {Key? key}) : super(key: key);
...
class _EditWishlistPageState extends State<EditWishlistPage> {
  ...
  void _submitWishlist(request, context) async {
    ...
    var response = await request
        .post('$baseUrl/wishlist/edit/${widget.id}/', {'note': _note.text}
    ...
    if (response['status']) {
      ...
      showDialog(
        context: context,
        builder: (context) => AlertDialog(
          title: const Text('Gagal mengedit wishlist'),
          content: Text(response['message']),
          actions: [
            TextButton(
              child: const Text('OK'),
